...
// Import kicks off a BACKGROUND CSV import of events. It streams the upload
...
func (h *EventHandler) Import(c *gin.Context) {
	...
	total, err := countCSVRowsEvent(tmpPath)
	if err != nil || total < 0 {
		os.Remove(tmpPath)
		c.JSON(http.StatusBadRequest, gin.H{
	...
	job := models.ImportJob{Resource: "events", Status: "processing", Total: total}
	...
	go h.runImportEvent(job.ID, tmpPath)
...
// countCSVRowsEvent counts data rows (excluding the header) without holding
// the file in memory.
func countCSVRowsEvent(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return -1, err
	}
...
// runImportEvent streams the temp CSV, creating events in batches and
...
func (h *EventHandler) runImportEvent(jobID, tmpPath string) {
	...
	flush := func() {
		...
		items := make([]models.Event, len(batch))
		for i := range batch {
	...
	for {
		...
		if v, ok := get(rec, "division"); ok && v != "" {
			...
			item.DivisionID = optionalStringPtr(rel.ID)
		}
		if v, ok := get(rec, "location"); ok {
			item.Location = v
		}
		if v, ok := get(rec, "banner_url"); ok {
			item.BannerUrl = v
		}
		if v, ok := get(rec, "allow_permission"); ok {
			item.AllowPermission = v == "true" || v == "1" || v == "yes"
		}
...
// Template returns a ready-to-fill CSV template (header row) for importing events.
...
func (h *EventHandler) Template(c *gin.Context) {
	c.Header("Content-Type", "text/csv")
	c.Header("Content-Disposition", `attachment; filename="events-template.csv"`)
	c.String(http.StatusOK, "title,description,division,location,banner_url,allow_permission,status\n")
}
