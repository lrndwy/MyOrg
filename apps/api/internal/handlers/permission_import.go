...
// Import kicks off a BACKGROUND CSV import of permissions. It streams the upload
...
func (h *PermissionHandler) Import(c *gin.Context) {
	...
	total, err := countCSVRowsPermission(tmpPath)
	if err != nil || total < 0 {
		os.Remove(tmpPath)
		c.JSON(http.StatusBadRequest, gin.H{
	...
	job := models.ImportJob{Resource: "permissions", Status: "processing", Total: total}
	...
	go h.runImportPermission(job.ID, tmpPath)
...
// countCSVRowsPermission counts data rows (excluding the header) without holding
// the file in memory.
func countCSVRowsPermission(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return -1, err
	}
...
// runImportPermission streams the temp CSV, creating permissions in batches and
...
func (h *PermissionHandler) runImportPermission(jobID, tmpPath string) {
	...
	flush := func() {
		...
		items := make([]models.Permission, len(batch))
		for i := range batch {
...
// Template returns a ready-to-fill CSV template (header row) for importing permissions.
...
func (h *PermissionHandler) Template(c *gin.Context) {
	c.Header("Content-Type", "text/csv")
	c.Header("Content-Disposition", `attachment; filename="permissions-template.csv"`)
	c.String(http.StatusOK, "code,module,description\n")
}
