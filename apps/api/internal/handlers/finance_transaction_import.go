...
// Import kicks off a BACKGROUND CSV import of finance_transactions. It streams the upload
...
func (h *FinanceTransactionHandler) Import(c *gin.Context) {
	...
	total, err := countCSVRowsFinanceTransaction(tmpPath)
	if err != nil || total < 0 {
		os.Remove(tmpPath)
		c.JSON(http.StatusBadRequest, gin.H{
	...
	job := models.ImportJob{Resource: "finance_transactions", Status: "processing", Total: total}
	...
	go h.runImportFinanceTransaction(job.ID, tmpPath)
...
// countCSVRowsFinanceTransaction counts data rows (excluding the header) without holding
// the file in memory.
func countCSVRowsFinanceTransaction(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return -1, err
	}
...
// runImportFinanceTransaction streams the temp CSV, creating finance_transactions in batches and
...
func (h *FinanceTransactionHandler) runImportFinanceTransaction(jobID, tmpPath string) {
	...
	flush := func() {
		...
		items := make([]models.FinanceTransaction, len(batch))
		for i := range batch {
	...
	for {
		...
		item := models.FinanceTransaction{}
		if v, ok := get(rec, "type"); ok {
			item.Type = v
		}
		if v, ok := get(rec, "amount"); ok {
		...
		if v, ok := get(rec, "proof_url"); ok {
			item.ProofUrl = v
		}
		if v, ok := get(rec, "category"); ok && v != "" {
			var rel models.FinanceCategory
			if err := h.DB.Where("name = ?", v).First(&rel).Error; err != nil {
				rel = models.FinanceCategory{Name: v}
				h.DB.Create(&rel)
			}
			item.CategoryID = rel.ID
		}
		if v, ok := get(rec, "recorded_by"); ok && v != "" {
			var rel models.User
			if err := h.DB.Where("email = ?", v).First(&rel).Error; err != nil {
				rel = models.User{Email: v}
				h.DB.Create(&rel)
			}
			item.RecordedByID = rel.ID
		}
...
// Template returns a ready-to-fill CSV template (header row) for importing finance_transactions.
...
func (h *FinanceTransactionHandler) Template(c *gin.Context) {
	c.Header("Content-Type", "text/csv")
	c.Header("Content-Disposition", `attachment; filename="finance-transactions-template.csv"`)
	c.String(http.StatusOK, "type,amount,description,proof_url,category,recorded_by\n")
}
