...
// Import kicks off a BACKGROUND CSV import of role_permissions. It streams the upload
...
func (h *RolePermissionHandler) Import(c *gin.Context) {
	...
	total, err := countCSVRowsRolePermission(tmpPath)
	if err != nil || total < 0 {
		os.Remove(tmpPath)
		c.JSON(http.StatusBadRequest, gin.H{
	...
	job := models.ImportJob{Resource: "role_permissions", Status: "processing", Total: total}
	...
	go h.runImportRolePermission(job.ID, tmpPath)
...
// countCSVRowsRolePermission counts data rows (excluding the header) without holding
// the file in memory.
func countCSVRowsRolePermission(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return -1, err
	}
...
// runImportRolePermission streams the temp CSV, creating role_permissions in batches and
...
func (h *RolePermissionHandler) runImportRolePermission(jobID, tmpPath string) {
	...
	flush := func() {
		...
		items := make([]models.RolePermission, len(batch))
		for i := range batch {
	...
	for {
		...
		item := models.RolePermission{}
		if v, ok := get(rec, "role"); ok && v != "" {
			var rel models.Role
			if err := h.DB.Where("name = ?", v).First(&rel).Error; err != nil {
				rel = models.Role{Name: v}
				h.DB.Create(&rel)
			}
			item.RoleID = rel.ID
		}
		if v, ok := get(rec, "permission"); ok && v != "" {
			var rel models.Permission
			if err := h.DB.Where("code = ?", v).First(&rel).Error; err != nil {
				rel = models.Permission{Code: v}
				h.DB.Create(&rel)
			}
			item.PermissionID = rel.ID
		}
...
// Template returns a ready-to-fill CSV template (header row) for importing role_permissions.
...
func (h *RolePermissionHandler) Template(c *gin.Context) {
	c.Header("Content-Type", "text/csv")
	c.Header("Content-Disposition", `attachment; filename="role-permissions-template.csv"`)
	c.String(http.StatusOK, "role,permission\n")
}
