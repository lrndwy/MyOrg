...
// Import kicks off a BACKGROUND CSV import of recruitment_custom_fields. It streams the upload
...
func (h *RecruitmentCustomFieldHandler) Import(c *gin.Context) {
	...
	total, err := countCSVRowsRecruitmentCustomField(tmpPath)
	if err != nil || total < 0 {
		os.Remove(tmpPath)
		c.JSON(http.StatusBadRequest, gin.H{
	...
	job := models.ImportJob{Resource: "recruitment_custom_fields", Status: "processing", Total: total}
	...
	go h.runImportRecruitmentCustomField(job.ID, tmpPath)
...
// countCSVRowsRecruitmentCustomField counts data rows (excluding the header) without holding
// the file in memory.
func countCSVRowsRecruitmentCustomField(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return -1, err
	}
...
// runImportRecruitmentCustomField streams the temp CSV, creating recruitment_custom_fields in batches and
...
func (h *RecruitmentCustomFieldHandler) runImportRecruitmentCustomField(jobID, tmpPath string) {
	...
	flush := func() {
		...
		items := make([]models.RecruitmentCustomField, len(batch))
		for i := range batch {
	...
	for {
		...
		if v, ok := get(rec, "field_label"); ok {
			item.FieldLabel = v
		}
		if v, ok := get(rec, "field_type"); ok {
			item.FieldType = v
		}
		if v, ok := get(rec, "is_required"); ok {
			item.IsRequired = v == "true" || v == "1" || v == "yes"
		}
		if v, ok := get(rec, "order_index"); ok {
			n, _ := strconv.Atoi(v)
			item.OrderIndex = n
		}
...
// Template returns a ready-to-fill CSV template (header row) for importing recruitment_custom_fields.
...
func (h *RecruitmentCustomFieldHandler) Template(c *gin.Context) {
	c.Header("Content-Type", "text/csv")
	c.Header("Content-Disposition", `attachment; filename="recruitment-custom-fields-template.csv"`)
	c.String(http.StatusOK, "recruitment,field_label,field_type,is_required,order_index\n")
}
