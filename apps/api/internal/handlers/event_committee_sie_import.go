...
// Import kicks off a BACKGROUND CSV import of event_committee_sies. It streams the upload
...
func (h *EventCommitteeSieHandler) Import(c *gin.Context) {
	...
	total, err := countCSVRowsEventCommitteeSie(tmpPath)
	if err != nil || total < 0 {
		os.Remove(tmpPath)
		c.JSON(http.StatusBadRequest, gin.H{
	...
	job := models.ImportJob{Resource: "event_committee_sies", Status: "processing", Total: total}
	...
	go h.runImportEventCommitteeSie(job.ID, tmpPath)
...
// countCSVRowsEventCommitteeSie counts data rows (excluding the header) without holding
// the file in memory.
func countCSVRowsEventCommitteeSie(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return -1, err
	}
...
// runImportEventCommitteeSie streams the temp CSV, creating event_committee_sies in batches and
...
func (h *EventCommitteeSieHandler) runImportEventCommitteeSie(jobID, tmpPath string) {
	...
	flush := func() {
		...
		items := make([]models.EventCommitteeSie, len(batch))
		for i := range batch {
...
// Template returns a ready-to-fill CSV template (header row) for importing event_committee_sies.
...
func (h *EventCommitteeSieHandler) Template(c *gin.Context) {
	c.Header("Content-Type", "text/csv")
	c.Header("Content-Disposition", `attachment; filename="event-committee-sies-template.csv"`)
	c.String(http.StatusOK, "event,name,description,order_index\n")
}
