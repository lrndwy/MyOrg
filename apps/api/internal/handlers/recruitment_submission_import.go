...
// Import kicks off a BACKGROUND CSV import of recruitment_submissions. It streams the upload
...
func (h *RecruitmentSubmissionHandler) Import(c *gin.Context) {
	...
	total, err := countCSVRowsRecruitmentSubmission(tmpPath)
	if err != nil || total < 0 {
		os.Remove(tmpPath)
		c.JSON(http.StatusBadRequest, gin.H{
	...
	job := models.ImportJob{Resource: "recruitment_submissions", Status: "processing", Total: total}
	...
	go h.runImportRecruitmentSubmission(job.ID, tmpPath)
...
// countCSVRowsRecruitmentSubmission counts data rows (excluding the header) without holding
// the file in memory.
func countCSVRowsRecruitmentSubmission(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return -1, err
	}
...
// runImportRecruitmentSubmission streams the temp CSV, creating recruitment_submissions in batches and
...
func (h *RecruitmentSubmissionHandler) runImportRecruitmentSubmission(jobID, tmpPath string) {
	...
	flush := func() {
		...
		items := make([]models.RecruitmentSubmission, len(batch))
		for i := range batch {
	...
	for {
		...
		item := models.RecruitmentSubmission{}
		if v, ok := get(rec, "recruitment"); ok && v != "" {
			var rel models.Recruitment
			if err := h.DB.Where("title = ?", v).First(&rel).Error; err != nil {
				rel = models.Recruitment{Title: v}
				h.DB.Create(&rel)
			}
			item.RecruitmentID = rel.ID
		}
		if v, ok := get(rec, "name"); ok {
			item.Name = v
		}
		if v, ok := get(rec, "nim"); ok {
			item.Nim = v
		}
		if v, ok := get(rec, "division_interest"); ok && v != "" {
			var rel models.Division
			if err := h.DB.Where("name = ?", v).First(&rel).Error; err != nil {
				rel = models.Division{Name: v}
				h.DB.Create(&rel)
			}
			item.DivisionInterestID = rel.ID
		}
		if v, ok := get(rec, "contact"); ok {
			item.Contact = v
		}
		if v, ok := get(rec, "custom_answers"); ok {
			item.CustomAnswers = jsonFromString(v)
		}
...
// Template returns a ready-to-fill CSV template (header row) for importing recruitment_submissions.
...
func (h *RecruitmentSubmissionHandler) Template(c *gin.Context) {
	c.Header("Content-Type", "text/csv")
	c.Header("Content-Disposition", `attachment; filename="recruitment-submissions-template.csv"`)
	c.String(http.StatusOK, "recruitment,name,nim,division_interest,contact,custom_answers,status\n")
}
