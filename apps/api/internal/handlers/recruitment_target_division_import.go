...
// Import kicks off a BACKGROUND CSV import of recruitment_target_divisions. It streams the upload
...
func (h *RecruitmentTargetDivisionHandler) Import(c *gin.Context) {
	...
	total, err := countCSVRowsRecruitmentTargetDivision(tmpPath)
	if err != nil || total < 0 {
		os.Remove(tmpPath)
		c.JSON(http.StatusBadRequest, gin.H{
	...
	job := models.ImportJob{Resource: "recruitment_target_divisions", Status: "processing", Total: total}
	...
	go h.runImportRecruitmentTargetDivision(job.ID, tmpPath)
...
// countCSVRowsRecruitmentTargetDivision counts data rows (excluding the header) without holding
// the file in memory.
func countCSVRowsRecruitmentTargetDivision(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return -1, err
	}
...
// runImportRecruitmentTargetDivision streams the temp CSV, creating recruitment_target_divisions in batches and
...
func (h *RecruitmentTargetDivisionHandler) runImportRecruitmentTargetDivision(jobID, tmpPath string) {
	...
	flush := func() {
		...
		items := make([]models.RecruitmentTargetDivision, len(batch))
		for i := range batch {
...
// Template returns a ready-to-fill CSV template (header row) for importing recruitment_target_divisions.
...
func (h *RecruitmentTargetDivisionHandler) Template(c *gin.Context) {
	c.Header("Content-Type", "text/csv")
	c.Header("Content-Disposition", `attachment; filename="recruitment-target-divisions-template.csv"`)
	c.String(http.StatusOK, "recruitment,division\n")
}
