...
// Import kicks off a BACKGROUND CSV import of letter_categories. It streams the upload
...
func (h *LetterCategoryHandler) Import(c *gin.Context) {
	...
	total, err := countCSVRowsLetterCategory(tmpPath)
	if err != nil || total < 0 {
		os.Remove(tmpPath)
		c.JSON(http.StatusBadRequest, gin.H{
	...
	job := models.ImportJob{Resource: "letter_categories", Status: "processing", Total: total}
	...
	go h.runImportLetterCategory(job.ID, tmpPath)
...
// countCSVRowsLetterCategory counts data rows (excluding the header) without holding
// the file in memory.
func countCSVRowsLetterCategory(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return -1, err
	}
...
// runImportLetterCategory streams the temp CSV, creating letter_categories in batches and
...
func (h *LetterCategoryHandler) runImportLetterCategory(jobID, tmpPath string) {
	...
	flush := func() {
		...
		items := make([]models.LetterCategory, len(batch))
		for i := range batch {
	...
	for {
		...
		if v, ok := get(rec, "code"); ok {
			item.Code = v
		}
		if v, ok := get(rec, "start_number"); ok {
			n, _ := strconv.Atoi(v)
			item.StartNumber = n
		}
		if v, ok := get(rec, "current_number"); ok {
			n, _ := strconv.Atoi(v)
			item.CurrentNumber = n
		}
		if v, ok := get(rec, "number_format_template"); ok {
			item.NumberFormatTemplate = v
		}
...
// Template returns a ready-to-fill CSV template (header row) for importing letter_categories.
...
func (h *LetterCategoryHandler) Template(c *gin.Context) {
	c.Header("Content-Type", "text/csv")
	c.Header("Content-Disposition", `attachment; filename="letter-categories-template.csv"`)
	c.String(http.StatusOK, "name,code,start_number,current_number,number_format_template\n")
}
