...
// Import kicks off a BACKGROUND CSV import of permission_requests. It streams the upload
...
func (h *PermissionRequestHandler) Import(c *gin.Context) {
	...
	total, err := countCSVRowsPermissionRequest(tmpPath)
	if err != nil || total < 0 {
		os.Remove(tmpPath)
		c.JSON(http.StatusBadRequest, gin.H{
	...
	job := models.ImportJob{Resource: "permission_requests", Status: "processing", Total: total}
	...
	go h.runImportPermissionRequest(job.ID, tmpPath)
...
// countCSVRowsPermissionRequest counts data rows (excluding the header) without holding
// the file in memory.
func countCSVRowsPermissionRequest(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return -1, err
	}
...
// runImportPermissionRequest streams the temp CSV, creating permission_requests in batches and
...
func (h *PermissionRequestHandler) runImportPermissionRequest(jobID, tmpPath string) {
	...
	flush := func() {
		...
		items := make([]models.PermissionRequest, len(batch))
		for i := range batch {
	...
	for {
		...
		if v, ok := get(rec, "reviewed_by"); ok && v != "" {
			var rel models.User
			if err := h.DB.Where("email = ?", v).First(&rel).Error; err != nil {
				rel = models.User{Email: v}
				h.DB.Create(&rel)
			}
			item.ReviewedByID = optionalStringPtr(rel.ID)
		}
		if v, ok := get(rec, "review_note"); ok {
			item.ReviewNote = v
		}
...
// Template returns a ready-to-fill CSV template (header row) for importing permission_requests.
...
func (h *PermissionRequestHandler) Template(c *gin.Context) {
	c.Header("Content-Type", "text/csv")
	c.Header("Content-Disposition", `attachment; filename="permission-requests-template.csv"`)
	c.String(http.StatusOK, "event,user,reason,proof_url,status,reviewed_by,review_note\n")
}
