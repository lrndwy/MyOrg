...
// Import kicks off a BACKGROUND CSV import of roles. It streams the upload
...
func (h *RoleHandler) Import(c *gin.Context) {
	...
	total, err := countCSVRowsRole(tmpPath)
	if err != nil || total < 0 {
		os.Remove(tmpPath)
		c.JSON(http.StatusBadRequest, gin.H{
	...
	job := models.ImportJob{Resource: "roles", Status: "processing", Total: total}
	...
	go h.runImportRole(job.ID, tmpPath)
...
// countCSVRowsRole counts data rows (excluding the header) without holding
// the file in memory.
func countCSVRowsRole(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return -1, err
	}
...
// runImportRole streams the temp CSV, creating roles in batches and
...
func (h *RoleHandler) runImportRole(jobID, tmpPath string) {
	...
	flush := func() {
		...
		items := make([]models.Role, len(batch))
		for i := range batch {
	...
	for {
		...
		if v, ok := get(rec, "is_system"); ok {
			item.IsSystem = v == "true" || v == "1" || v == "yes"
		}
...
// Template returns a ready-to-fill CSV template (header row) for importing roles.
...
func (h *RoleHandler) Template(c *gin.Context) {
	c.Header("Content-Type", "text/csv")
	c.Header("Content-Disposition", `attachment; filename="roles-template.csv"`)
	c.String(http.StatusOK, "name,description,is_system\n")
}
