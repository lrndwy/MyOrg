...
// Import kicks off a BACKGROUND CSV import of letter_templates. It streams the upload
...
func (h *LetterTemplateHandler) Import(c *gin.Context) {
	...
	total, err := countCSVRowsLetterTemplate(tmpPath)
	if err != nil || total < 0 {
		os.Remove(tmpPath)
		c.JSON(http.StatusBadRequest, gin.H{
	...
	job := models.ImportJob{Resource: "letter_templates", Status: "processing", Total: total}
	...
	go h.runImportLetterTemplate(job.ID, tmpPath)
...
// countCSVRowsLetterTemplate counts data rows (excluding the header) without holding
// the file in memory.
func countCSVRowsLetterTemplate(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return -1, err
	}
...
// runImportLetterTemplate streams the temp CSV, creating letter_templates in batches and
...
func (h *LetterTemplateHandler) runImportLetterTemplate(jobID, tmpPath string) {
	...
	flush := func() {
		...
		items := make([]models.LetterTemplate, len(batch))
		for i := range batch {
	...
	for {
		...
		if v, ok := get(rec, "template_url"); ok {
			item.TemplateUrl = v
		}
...
// Template returns a ready-to-fill CSV template (header row) for importing letter_templates.
...
func (h *LetterTemplateHandler) Template(c *gin.Context) {
	c.Header("Content-Type", "text/csv")
	c.Header("Content-Disposition", `attachment; filename="letter-templates-template.csv"`)
	c.String(http.StatusOK, "name,category,template_url\n")
}
