...
// Import kicks off a BACKGROUND CSV import of recruitments. It streams the upload
...
func (h *RecruitmentHandler) Import(c *gin.Context) {
	...
	total, err := countCSVRowsRecruitment(tmpPath)
	if err != nil || total < 0 {
		os.Remove(tmpPath)
		c.JSON(http.StatusBadRequest, gin.H{
	...
	job := models.ImportJob{Resource: "recruitments", Status: "processing", Total: total}
	...
	go h.runImportRecruitment(job.ID, tmpPath)
...
// countCSVRowsRecruitment counts data rows (excluding the header) without holding
// the file in memory.
func countCSVRowsRecruitment(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return -1, err
	}
...
// runImportRecruitment streams the temp CSV, creating recruitments in batches and
...
func (h *RecruitmentHandler) runImportRecruitment(jobID, tmpPath string) {
	...
	flush := func() {
		...
		items := make([]models.Recruitment, len(batch))
		for i := range batch {
...
// Template returns a ready-to-fill CSV template (header row) for importing recruitments.
...
func (h *RecruitmentHandler) Template(c *gin.Context) {
	c.Header("Content-Type", "text/csv")
	c.Header("Content-Disposition", `attachment; filename="recruitments-template.csv"`)
	c.String(http.StatusOK, "title,description,status\n")
}
