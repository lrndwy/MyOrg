...
// Import kicks off a BACKGROUND CSV import of event_sub_events. It streams the upload
...
func (h *EventSubEventHandler) Import(c *gin.Context) {
	...
	total, err := countCSVRowsEventSubEvent(tmpPath)
	if err != nil || total < 0 {
		os.Remove(tmpPath)
		c.JSON(http.StatusBadRequest, gin.H{
	...
	job := models.ImportJob{Resource: "event_sub_events", Status: "processing", Total: total}
	...
	go h.runImportEventSubEvent(job.ID, tmpPath)
...
// countCSVRowsEventSubEvent counts data rows (excluding the header) without holding
// the file in memory.
func countCSVRowsEventSubEvent(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return -1, err
	}
...
// runImportEventSubEvent streams the temp CSV, creating event_sub_events in batches and
...
func (h *EventSubEventHandler) runImportEventSubEvent(jobID, tmpPath string) {
	...
	flush := func() {
		...
		items := make([]models.EventSubEvent, len(batch))
		for i := range batch {
	...
	for {
		...
		item := models.EventSubEvent{}
		if v, ok := get(rec, "event"); ok && v != "" {
			var rel models.Event
			if err := h.DB.Where("title = ?", v).First(&rel).Error; err != nil {
				rel = models.Event{Title: v}
				h.DB.Create(&rel)
			}
			item.EventID = rel.ID
		}
		if v, ok := get(rec, "sie"); ok && v != "" {
			var rel models.EventCommitteeSie
			if err := h.DB.Where("name = ?", v).First(&rel).Error; err != nil {
				rel = models.EventCommitteeSie{Name: v}
				h.DB.Create(&rel)
			}
			item.SieID = rel.ID
		}
		...
		if v, ok := get(rec, "description"); ok {
			item.Description = v
		}
		if v, ok := get(rec, "location"); ok {
			item.Location = v
		}
		if v, ok := get(rec, "ketua_pelaksana"); ok && v != "" {
			var rel models.User
			if err := h.DB.Where("email = ?", v).First(&rel).Error; err != nil {
				rel = models.User{Email: v}
				h.DB.Create(&rel)
			}
			item.KetuaPelaksanaID = rel.ID
		}
		if v, ok := get(rec, "attendance_mode"); ok {
			item.AttendanceMode = v
		}
		if v, ok := get(rec, "minutes_url"); ok {
			item.MinutesUrl = v
		}
...
// Template returns a ready-to-fill CSV template (header row) for importing event_sub_events.
...
func (h *EventSubEventHandler) Template(c *gin.Context) {
	c.Header("Content-Type", "text/csv")
	c.Header("Content-Disposition", `attachment; filename="event-sub-events-template.csv"`)
	c.String(http.StatusOK, "event,sie,title,description,location,ketua_pelaksana,attendance_mode,minutes_url,status\n")
}
