...
// Import kicks off a BACKGROUND CSV import of finance_categories. It streams the upload
...
func (h *FinanceCategoryHandler) Import(c *gin.Context) {
	...
	total, err := countCSVRowsFinanceCategory(tmpPath)
	if err != nil || total < 0 {
		os.Remove(tmpPath)
		c.JSON(http.StatusBadRequest, gin.H{
	...
	job := models.ImportJob{Resource: "finance_categories", Status: "processing", Total: total}
	...
	go h.runImportFinanceCategory(job.ID, tmpPath)
...
// countCSVRowsFinanceCategory counts data rows (excluding the header) without holding
// the file in memory.
func countCSVRowsFinanceCategory(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return -1, err
	}
...
// runImportFinanceCategory streams the temp CSV, creating finance_categories in batches and
...
func (h *FinanceCategoryHandler) runImportFinanceCategory(jobID, tmpPath string) {
	...
	flush := func() {
		...
		items := make([]models.FinanceCategory, len(batch))
		for i := range batch {
...
// Template returns a ready-to-fill CSV template (header row) for importing finance_categories.
...
func (h *FinanceCategoryHandler) Template(c *gin.Context) {
	c.Header("Content-Type", "text/csv")
	c.Header("Content-Disposition", `attachment; filename="finance-categories-template.csv"`)
	c.String(http.StatusOK, "name,type,description\n")
}
