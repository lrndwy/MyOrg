...
// Import kicks off a BACKGROUND CSV import of divisions. It streams the upload
...
func (h *DivisionHandler) Import(c *gin.Context) {
	...
	total, err := countCSVRowsDivision(tmpPath)
	if err != nil || total < 0 {
		os.Remove(tmpPath)
		c.JSON(http.StatusBadRequest, gin.H{
	...
	job := models.ImportJob{Resource: "divisions", Status: "processing", Total: total}
	...
	go h.runImportDivision(job.ID, tmpPath)
...
// countCSVRowsDivision counts data rows (excluding the header) without holding
// the file in memory.
func countCSVRowsDivision(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return -1, err
	}
...
// runImportDivision streams the temp CSV, creating divisions in batches and
...
func (h *DivisionHandler) runImportDivision(jobID, tmpPath string) {
	...
	flush := func() {
		...
		items := make([]models.Division, len(batch))
		for i := range batch {
...
// Template returns a ready-to-fill CSV template (header row) for importing divisions.
...
func (h *DivisionHandler) Template(c *gin.Context) {
	c.Header("Content-Type", "text/csv")
	c.Header("Content-Disposition", `attachment; filename="divisions-template.csv"`)
	c.String(http.StatusOK, "name,description\n")
}
