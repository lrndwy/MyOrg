...
// Import kicks off a BACKGROUND CSV import of sub_event_attendances. It streams the upload
...
func (h *SubEventAttendanceHandler) Import(c *gin.Context) {
	...
	total, err := countCSVRowsSubEventAttendance(tmpPath)
	if err != nil || total < 0 {
		os.Remove(tmpPath)
		c.JSON(http.StatusBadRequest, gin.H{
	...
	job := models.ImportJob{Resource: "sub_event_attendances", Status: "processing", Total: total}
	...
	go h.runImportSubEventAttendance(job.ID, tmpPath)
...
// countCSVRowsSubEventAttendance counts data rows (excluding the header) without holding
// the file in memory.
func countCSVRowsSubEventAttendance(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return -1, err
	}
...
// runImportSubEventAttendance streams the temp CSV, creating sub_event_attendances in batches and
...
func (h *SubEventAttendanceHandler) runImportSubEventAttendance(jobID, tmpPath string) {
	...
	flush := func() {
		...
		items := make([]models.SubEventAttendance, len(batch))
		for i := range batch {
	...
	for {
		...
		item := models.SubEventAttendance{}
		if v, ok := get(rec, "sub_event"); ok && v != "" {
			var rel models.EventSubEvent
			if err := h.DB.Where("title = ?", v).First(&rel).Error; err != nil {
				rel = models.EventSubEvent{Title: v}
				h.DB.Create(&rel)
			}
			item.SubEventID = rel.ID
		}
		if v, ok := get(rec, "user"); ok && v != "" {
			var rel models.User
		...
		if v, ok := get(rec, "selfie_url"); ok {
			item.SelfieUrl = v
		}
		if v, ok := get(rec, "signature_url"); ok {
			item.SignatureUrl = v
		}
		if v, ok := get(rec, "marked_by"); ok && v != "" {
			var rel models.User
			if err := h.DB.Where("email = ?", v).First(&rel).Error; err != nil {
				rel = models.User{Email: v}
				h.DB.Create(&rel)
			}
			item.MarkedByID = &rel.ID
		}
...
// Template returns a ready-to-fill CSV template (header row) for importing sub_event_attendances.
...
func (h *SubEventAttendanceHandler) Template(c *gin.Context) {
	c.Header("Content-Type", "text/csv")
	c.Header("Content-Disposition", `attachment; filename="sub-event-attendances-template.csv"`)
	c.String(http.StatusOK, "sub_event,user,status,selfie_url,signature_url,marked_by\n")
}
