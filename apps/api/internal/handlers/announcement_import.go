...
// Import kicks off a BACKGROUND CSV import of announcements. It streams the upload
...
func (h *AnnouncementHandler) Import(c *gin.Context) {
	...
	total, err := countCSVRowsAnnouncement(tmpPath)
	if err != nil || total < 0 {
		os.Remove(tmpPath)
		c.JSON(http.StatusBadRequest, gin.H{
	...
	job := models.ImportJob{Resource: "announcements", Status: "processing", Total: total}
	...
	go h.runImportAnnouncement(job.ID, tmpPath)
...
// countCSVRowsAnnouncement counts data rows (excluding the header) without holding
// the file in memory.
func countCSVRowsAnnouncement(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return -1, err
	}
...
// runImportAnnouncement streams the temp CSV, creating announcements in batches and
...
func (h *AnnouncementHandler) runImportAnnouncement(jobID, tmpPath string) {
	...
	flush := func() {
		...
		items := make([]models.Announcement, len(batch))
		for i := range batch {
	...
	for {
		...
		if v, ok := get(rec, "target_type"); ok {
			item.TargetType = v
		}
		if v, ok := get(rec, "target_division"); ok && v != "" {
			var rel models.Division
			if err := h.DB.Where("name = ?", v).First(&rel).Error; err != nil {
				rel = models.Division{Name: v}
				h.DB.Create(&rel)
			}
			item.TargetDivisionID = optionalStringPtr(rel.ID)
		}
...
// Template returns a ready-to-fill CSV template (header row) for importing announcements.
...
func (h *AnnouncementHandler) Template(c *gin.Context) {
	c.Header("Content-Type", "text/csv")
	c.Header("Content-Disposition", `attachment; filename="announcements-template.csv"`)
	c.String(http.StatusOK, "title,content,target_type,target_division\n")
}
