...
// Import kicks off a BACKGROUND CSV import of letters. It streams the upload
...
func (h *LetterHandler) Import(c *gin.Context) {
	...
	total, err := countCSVRowsLetter(tmpPath)
	if err != nil || total < 0 {
		os.Remove(tmpPath)
		c.JSON(http.StatusBadRequest, gin.H{
	...
	job := models.ImportJob{Resource: "letters", Status: "processing", Total: total}
	...
	go h.runImportLetter(job.ID, tmpPath)
...
// countCSVRowsLetter counts data rows (excluding the header) without holding
// the file in memory.
func countCSVRowsLetter(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return -1, err
	}
...
// runImportLetter streams the temp CSV, creating letters in batches and
...
func (h *LetterHandler) runImportLetter(jobID, tmpPath string) {
	...
	flush := func() {
		...
		items := make([]models.Letter, len(batch))
		for i := range batch {
	...
	for {
		...
		if v, ok := get(rec, "category"); ok && v != "" {
			var rel models.LetterCategory
			if err := h.DB.Where("name = ?", v).First(&rel).Error; err != nil {
				rel = models.LetterCategory{Name: v}
				h.DB.Create(&rel)
			}
			item.CategoryID = rel.ID
		}
		if v, ok := get(rec, "letter_code"); ok {
			item.LetterCode = v
		}
		if v, ok := get(rec, "subject"); ok {
			item.Subject = v
		}
		if v, ok := get(rec, "sender"); ok {
			item.Sender = v
		}
		if v, ok := get(rec, "recipient"); ok {
			item.Recipient = v
		}
		if v, ok := get(rec, "content"); ok {
			item.Content = v
		}
		if v, ok := get(rec, "description"); ok && item.Content == "" {
			item.Content = v
		}
		if v, ok := get(rec, "document_url"); ok {
			item.DocumentUrl = v
		}
		if v, ok := get(rec, "attachment_url"); ok && item.DocumentUrl == "" {
			item.DocumentUrl = v
		}
...
// Template returns a ready-to-fill CSV template (header row) for importing letters.
...
func (h *LetterHandler) Template(c *gin.Context) {
	c.Header("Content-Type", "text/csv")
	c.Header("Content-Disposition", `attachment; filename="letters-template.csv"`)
	c.String(http.StatusOK, "type,category,letter_code,subject,sender,recipient,description,attachment_url\n")
}
