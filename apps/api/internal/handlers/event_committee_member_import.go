...
// Import kicks off a BACKGROUND CSV import of event_committee_members. It streams the upload
...
func (h *EventCommitteeMemberHandler) Import(c *gin.Context) {
	...
	total, err := countCSVRowsEventCommitteeMember(tmpPath)
	if err != nil || total < 0 {
		os.Remove(tmpPath)
		c.JSON(http.StatusBadRequest, gin.H{
	...
	job := models.ImportJob{Resource: "event_committee_members", Status: "processing", Total: total}
	...
	go h.runImportEventCommitteeMember(job.ID, tmpPath)
...
// countCSVRowsEventCommitteeMember counts data rows (excluding the header) without holding
// the file in memory.
func countCSVRowsEventCommitteeMember(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return -1, err
	}
...
// runImportEventCommitteeMember streams the temp CSV, creating event_committee_members in batches and
...
func (h *EventCommitteeMemberHandler) runImportEventCommitteeMember(jobID, tmpPath string) {
	...
	flush := func() {
		...
		items := make([]models.EventCommitteeMember, len(batch))
		for i := range batch {
...
// Template returns a ready-to-fill CSV template (header row) for importing event_committee_members.
...
func (h *EventCommitteeMemberHandler) Template(c *gin.Context) {
	c.Header("Content-Type", "text/csv")
	c.Header("Content-Disposition", `attachment; filename="event-committee-members-template.csv"`)
	c.String(http.StatusOK, "sie,user,role\n")
}
