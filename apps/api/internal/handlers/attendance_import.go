...
// Import kicks off a BACKGROUND CSV import of attendances. It streams the upload
...
func (h *AttendanceHandler) Import(c *gin.Context) {
	...
	total, err := countCSVRowsAttendance(tmpPath)
	if err != nil || total < 0 {
		os.Remove(tmpPath)
		c.JSON(http.StatusBadRequest, gin.H{
	...
	job := models.ImportJob{Resource: "attendances", Status: "processing", Total: total}
	...
	go h.runImportAttendance(job.ID, tmpPath)
...
// countCSVRowsAttendance counts data rows (excluding the header) without holding
// the file in memory.
func countCSVRowsAttendance(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return -1, err
	}
...
// runImportAttendance streams the temp CSV, creating attendances in batches and
...
func (h *AttendanceHandler) runImportAttendance(jobID, tmpPath string) {
	...
	flush := func() {
		...
		items := make([]models.Attendance, len(batch))
		for i := range batch {
...
// Template returns a ready-to-fill CSV template (header row) for importing attendances.
...
func (h *AttendanceHandler) Template(c *gin.Context) {
	c.Header("Content-Type", "text/csv")
	c.Header("Content-Disposition", `attachment; filename="attendances-template.csv"`)
	c.String(http.StatusOK, "event,user,status,selfie_url,signature_url\n")
}
