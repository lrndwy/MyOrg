...
// Import kicks off a BACKGROUND CSV import of violations. It streams the upload
...
func (h *ViolationHandler) Import(c *gin.Context) {
	...
	total, err := countCSVRowsViolation(tmpPath)
	if err != nil || total < 0 {
		os.Remove(tmpPath)
		c.JSON(http.StatusBadRequest, gin.H{
	...
	job := models.ImportJob{Resource: "violations", Status: "processing", Total: total}
	...
	go h.runImportViolation(job.ID, tmpPath)
...
// countCSVRowsViolation counts data rows (excluding the header) without holding
// the file in memory.
func countCSVRowsViolation(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return -1, err
	}
...
// runImportViolation streams the temp CSV, creating violations in batches and
...
func (h *ViolationHandler) runImportViolation(jobID, tmpPath string) {
	...
	flush := func() {
		...
		items := make([]models.Violation, len(batch))
		for i := range batch {
	...
	for {
		...
		if v, ok := get(rec, "violation_type"); ok {
			item.ViolationType = v
		}
		if v, ok := get(rec, "description"); ok {
			item.Description = v
		}
		if v, ok := get(rec, "sp_level"); ok {
			item.SpLevel = v
		}
		if v, ok := get(rec, "document_url"); ok {
			item.DocumentUrl = v
		}
		if v, ok := get(rec, "issued_by"); ok && v != "" {
			var rel models.User
			if err := h.DB.Where("email = ?", v).First(&rel).Error; err != nil {
				rel = models.User{Email: v}
				h.DB.Create(&rel)
			}
			item.IssuedByID = rel.ID
		}
...
// Template returns a ready-to-fill CSV template (header row) for importing violations.
...
func (h *ViolationHandler) Template(c *gin.Context) {
	c.Header("Content-Type", "text/csv")
	c.Header("Content-Disposition", `attachment; filename="violations-template.csv"`)
	c.String(http.StatusOK, "user,violation_type,description,sp_level,document_url,issued_by\n")
}
