...
// Import kicks off a BACKGROUND CSV import of organization_settings. It streams the upload
...
func (h *OrganizationSettingHandler) Import(c *gin.Context) {
	...
	total, err := countCSVRowsOrganizationSetting(tmpPath)
	if err != nil || total < 0 {
		os.Remove(tmpPath)
		c.JSON(http.StatusBadRequest, gin.H{
	...
	job := models.ImportJob{Resource: "organization_settings", Status: "processing", Total: total}
	...
	go h.runImportOrganizationSetting(job.ID, tmpPath)
...
// countCSVRowsOrganizationSetting counts data rows (excluding the header) without holding
// the file in memory.
func countCSVRowsOrganizationSetting(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return -1, err
	}
...
// runImportOrganizationSetting streams the temp CSV, creating organization_settings in batches and
...
func (h *OrganizationSettingHandler) runImportOrganizationSetting(jobID, tmpPath string) {
	...
	flush := func() {
		...
		items := make([]models.OrganizationSetting, len(batch))
		for i := range batch {
	...
	for {
		...
		item := models.OrganizationSetting{}
		if v, ok := get(rec, "web_name"); ok {
			item.WebName = v
		}
		if v, ok := get(rec, "logo_url"); ok {
			item.LogoUrl = v
		}
		if v, ok := get(rec, "icon_url"); ok {
			item.IconUrl = v
		}
		if v, ok := get(rec, "theme"); ok {
			item.Theme = v
		}
		if v, ok := get(rec, "allow_self_register"); ok {
			item.AllowSelfRegister = v == "true" || v == "1" || v == "yes"
		}
		if v, ok := get(rec, "allow_cross_division_events_view"); ok {
			item.AllowCrossDivisionEventsView = v == "true" || v == "1" || v == "yes"
		}
...
// Template returns a ready-to-fill CSV template (header row) for importing organization_settings.
...
func (h *OrganizationSettingHandler) Template(c *gin.Context) {
	c.Header("Content-Type", "text/csv")
	c.Header("Content-Disposition", `attachment; filename="organization-settings-template.csv"`)
	c.String(http.StatusOK, "web_name,logo_url,icon_url,theme,allow_self_register,allow_cross_division_events_view\n")
}
